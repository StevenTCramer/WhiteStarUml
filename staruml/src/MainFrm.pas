...
type
  ...
  private
    ...
    ActionProcessor: PActionProcessor;
    FFileName: string;
    ...
    FOnPaletteNavBarCustomDrawLink: TdxNavBarCustomDrawLinkEvent;
  ...
  public
    ...
    procedure ActivateSelectHandler;
    ...
    property OnPaletteNavBarCustomDrawLink: TdxNavBarCustomDrawLinkEvent read FOnPaletteNavBarCustomDrawLink write FOnPaletteNavBarCustomDrawLink;
    // Event Fields (On InspectorForm Attachment Toolbar's events)
...
uses
  StarUMLApp, UMLFacto, NLS, NLS_StarUML,
...
begin
  ...
  ColorDialog.Free;
  if ActionProcessor <> nil then ActionProcessor.Free;
  inherited;
end;
...
procedure TMainForm.SelectSelectNavBarItem(NavBarGroup: TdxNavBarGroup);
var
...
begin
  ActionProcessor := PActionProcessor.Create;
...
procedure TMainForm.ActivateSelectHandler;
begin
  if PaletteNavBar.ActiveGroup <> nil then
    SelectSelectNavBarItem(PaletteNavBar.ActiveGroup);
  ActionProcessor.ActivateHandler('SelectHandler');
end;
...
procedure TMainForm.HandlerButtonClick(Sender: TObject);
begin
...
procedure TMainForm.PaletteNavBarActiveGroupChanged(Sender: TObject);
begin
  if ActionProcessor <> nil then
...
procedure TMainForm.PaletteNavBarCustomDrawLink(Sender: TObject; ACanvas: TCanvas;
  AViewInfo: TdxNavBarLinkViewInfo; var AHandled: Boolean);
begin
  if Assigned(FOnPaletteNavBarCustomDrawLink) then
    FOnPaletteNavBarCustomDrawLink(Self, ACanvas, AViewInfo, AHandled);
end;
