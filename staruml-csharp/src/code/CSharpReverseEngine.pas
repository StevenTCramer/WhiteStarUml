...
begin
  ...
  while (XID <> PGMR_NODE_NOT_EXISTS) do begin
    ...
    begin
      if Pos(CSHARP_DOC_BEGIN, FProGMR.GetValue(XID)) > 0 then
      begin
        BeginIdx := Pos(CSHARP_DOC_BEGIN, FProGMR.GetValue(XID));
        EndIdx := Pos(CSHARP_DOC_END,FProGMR.GetValue(XID));
...
begin
  Result := Copy(ATrimValue, Pos(FRootUMLPackage.Pathname ,ATrimValue), Length(ATrimValue) - Length(FRootUMLPackage.Pathname));
end;
